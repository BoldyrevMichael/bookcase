 ...
 * Токены для тестов.
 *
 * <p>Ключ выпускается на месте, а отдаёт его поднятый в этом же процессе крошечный http-сервер:
 * сервис забирает ключи ровно так же, как забирал бы их у настоящего Keycloak, но владельца можно
 * назначить любого — а это и нужно, чтобы проверить, что чужие файлы не видны.
 */
final class TestIdentity {
    ...
    /** Токен владельца с заданным идентификатором. */
    static String tokenFor(String ownerId) {
        return tokenFor(ownerId, java.util.List.of());
    }

    /**
     * Токен с ролями realm.
     *
     * <p>Так выглядит токен служебной учётной записи: права в нём приходят не в {@code scope}, а в
     * {@code realm_access.roles} — Keycloak формирует {@code scope} сам.
     */
    static String tokenFor(String ownerId, java.util.List<String> realmRoles) {
        JWTClaimsSet claims =
                new JWTClaimsSet.Builder()
                        ...
                        .claim("realm_access", java.util.Map.of("roles", realmRoles))
                        .build();
        SignedJWT jwt =
                new SignedJWT(
